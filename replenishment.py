...
if __name__ == "__main__":
    ...
    has_reinvestment_feature = df_os.iloc[4,1]
    reinvestment_period = df_os.iloc[1,1]
    SOFR = df_os.iloc[3,1]

    replenishment_period = df_os.iloc[6,1]
    has_replenishment_feature = df_os.iloc[5,1]
    ...
    clo = CLO(ramp_up, reinvestment_period, first_payment_date, replenishment_period)
    ...
    while months_passed in range(longest_duration): # longest duration 
      ...
      while portfolio_index < len(loan_portfolio.get_active_portfolio()):
        ...
        if principal_pay != 0: 
           ...
           # reinvestment only (but replenishment is located inside if the feature is TRUE)
           if has_reinvestment_feature:
              ...
              # both reinvestment only and replenishment only 
              else:
                 # we just have to check if the months_passed > reinvestment_period
                 # because that means replenishment should definitely start
                 if months_passed > reinvestment_period and has_replenishment_feature:
                    # replenishment is a go
                    if months_passed <= replenishment_period:
                       # is get_term_length still the right function or does it need its own replenishment_term_length thing going on
                       # when both features are present he didn't explicitly say the conditions applied
                       loan_portfolio.add_new_loan(beginning_bal, margin, months_passed, ramp = False)
                       # not sure if this is what happens during replenishment, but there's a question to ask
                    else:
                       clo.get_tranches()[0].subtract_size(beginning_bal)
                       # he says works same way (not sure if he was referring to this but pretty sure)
                    break
                 clo.get_tranches()[0].subtract_size(beginning_bal)
           if has_replenishment_feature and not has_reinvestment_feature:
              if months_passed > replenishment_period or clo.get_remaining_replenishment_amount() == 0:
                 # 1st condition: hits term length >> terminate feature
                 # OR
                 # 2nd condition: replenishment amount used up >> terminate feature
                 # TODO: create a get_remaining_replenishment_amount (i think loan class)
                 # take amount from spread sheet and use subtractor/setter, getter to hold onto the cumulative amount
                 # (not sure how to simulate those payments)
                 # at this point not sure what to do with this whole part
                 break
              else:
                 # never specified what replenishment actually does so now im confused
                 # work the same but as reinvestment, only the two have different conditions??
