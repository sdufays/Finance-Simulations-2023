...
def loan_waterfall(subtract_value, tranches):
    ...
    if subtract_value > 0:
        raise ValueError(
            "Not enough total size in all tranches to cover the subtraction."
        )
...
def run_simulation(
    ...
    loan_ids = list(
        range(1, 1 + len(loan_portfolio.get_active_portfolio()))
    )  # 21 loan IDs
    months = list(range(0, longest_duration))
    loan_index = pd.MultiIndex.from_product(
        [loan_ids, months], names=["Loan ID", "Months Passed"]
    )
    ...
    tranche_index = pd.MultiIndex.from_product(
        [tranche_names, months], names=["Tranche Name", "Month"]
    )
    tranche_df = pd.DataFrame(
        index=tranche_index,
        columns=["Interest Payment", "Principal Payment", "Tranche Size"],
    )
    ...
    for loan in loan_portfolio.get_active_portfolio():
        loan_term_df.loc[loan_term_df.shape[0]] = [
            loan.get_loan_id(),
    ...
    while months_passed in range(longest_duration):
        ...
        if months_passed == 1:
            ...
            if extra_balance > 0:
                loan_portfolio.add_new_loan(
                    extra_balance, margin, months_passed, ramp=True
                )
        ...
        while portfolio_index < len(loan_portfolio.get_active_portfolio()):
            ...
            loan_df.loc[
                (loan.get_loan_id(), months_passed), "Beginning Balance"
            ] = beginning_bal
            loan_df.loc[
                (loan.get_loan_id(), months_passed), "Interest Income"
            ] = interest_inc
            loan_df.loc[
                (loan.get_loan_id(), months_passed), "Principal Paydown"
            ] = principal_pay
            loan_df.loc[
                (loan.get_loan_id(), months_passed), "Ending Balance"
            ] = ending_bal
            loan_df.loc[
                (loan.get_loan_id(), months_passed), "Current Month"
            ] = current_month
            ...
            if principal_pay != 0:
                ...
                reinvestment_bool = (
                    (clo.get_reinv_bool())
                    and (months_passed <= clo.get_reinv_period())
                    and (months_passed == loan.get_term_length())
                )
                replenishment_bool = (
                    (clo.get_replen_bool() and not clo.get_reinv_bool())
                    and (
                        months_passed <= clo.get_replen_period()
                        and replen_cumulative < clo.get_replen_amount()
                    )
                    and (months_passed == loan.get_term_length())
                )
                replen_after_reinv_bool = (
                    (clo.get_reinv_bool() and clo.get_replen_bool())
                    and (months_passed > clo.get_reinv_period())
                    and (
                        replen_months < clo.get_replen_period()
                        and replen_cumulative < clo.get_replen_amount()
                    )
                    and (months_passed == loan.get_term_length())
                )
                ...
                if reinvestment_bool:
                    loan_portfolio.add_new_loan(
                        beginning_bal, margin, months_passed, ramp=False
                    )
                elif replenishment_bool:
                    loan_value = min(
                        beginning_bal, clo.get_replen_amount() - replen_cumulative
                    )
                    loan_portfolio.add_new_loan(
                        loan_value, margin, months_passed, ramp=False
                    )
        ...
        clo.append_cashflow(
            months_passed, upfront_costs, days, SOFR, tranche_df, terminate_next
        )
        ...
        if case != "market aware" and clo.get_tranches()[0].get_size() <= threshold:
            terminate_next = True
        elif case == "market aware" and (
            clo.current_clo_size(tranche_df, months_passed)
    ...
    wa_cof = (
        npf.irr(clo.get_total_cashflows()) * 12 * 360 / 365 - SOFR
    ) * 100  # in bps
    ...
    clo_interest_cost = initial_clo_tob * (
        wa_cof / 100 + SOFR
    )  # interest we pay to tranches
    net_equity_amt = (
        loan_portfolio.get_initial_deal_size() - initial_clo_tob
    )  # total amount of loans - amount offered as tranches
    equity_net_spread = (
        collateral_income - clo_interest_cost
    ) / net_equity_amt  # excess equity availalbe
    # origination fee add on (fee for creating the clo)
    origination_fee = (
        loan_portfolio.get_initial_deal_size()
        * 0.01
        / (net_equity_amt * deal_call_month)
    )  # remember in simulation to put deal_call_mos[trial]
    ...
    if case != "market aware":
        ...
        output_dataframe.loc[
            (case_name, trial_index), "Deal Call Month"
        ] = deal_call_month
        ...
        output_dataframe.loc[
            (case_name, trial_index), "Projected Equity Yield"
        ] = projected_equity_yield
    else:
        ...
        output_dataframe.loc[
            trial_index, "Projected Equity Yield"
        ] = projected_equity_yield
...
if __name__ == "__main__":
    ...
    base = [0.33, 0.33, 0.34]
    downside = [0.30, 0.25, 0.45]
    upside = [0.40, 0.35, 0.25]
    ...
    index = pd.MultiIndex.from_product(
        [cases, trial_numbers], names=["Case", "Trial Number"]
    )
    ...
    df_os = pd.read_excel(
        excel_file_folder + excel_file_path,
        sheet_name="Other Specifications",
        header=None,
    )
    ...
    df_uc = pd.read_excel(
        excel_file_folder + excel_file_path, sheet_name="Upfront Costs", header=None
    )
    ...
    df_cs = pd.read_excel(
        excel_file_folder + excel_file_path, sheet_name="Capital Stack"
    )

    # read excel file for loans
    df_cp = pd.read_excel(
        excel_file_folder + excel_file_path, sheet_name="Collateral Portfolio"
    )
    ...
    if has_market_aware:
        for run in range(NUM_TRIALS):
            ...
            for index_t, row_t in df_cs.iterrows():
                tranche_data = row_t[
                    ["Name", "Rating", "Offered", "Size", "Spread (bps)", "Price"]
                ]
                clo_obj.add_tranche(
                    tranche_data[0],
                    tranche_data[1],
            ...
            for index_l, row_l in df_cp.iterrows():
                loan_data = row_l[
                    [
                        "Loan ID",
                        "Collateral Interest UPB",
                        "Margin",
                        "Index Floor",
                        "Loan Term (rem)",
                        "First Extension Period (mo)",
                        "Open Prepayment Period",
                    ]
                ]
                loan_portfolio_obj.add_initial_loan(
                    loan_data[0],
                    loan_data[1],
                    loan_data[2],
                    loan_data[3],
            ...
            total_upfront_costs = clo_obj.get_upfront_costs(
                placement_percent,
                legal,
                accounting,
                trustee,
